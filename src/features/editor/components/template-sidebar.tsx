...
import { useGetTemplates, ResponseType } from "@/features/projects/use-get-templates";
...
export const TemplateSidebar = ({
  ...
  const [ConfirmDialog, confirm] = useConfirm(
    "Are you sure?",
    "You arre about to replace the current project with this template"
  )
            ...
            {data && data.map((template) => {
              return (
                ...
                >
                  ...
                  />
                  {template.isPremium && (
                    <div className="absolute top-2 right-2 size-8 items-center flex justify-center
                    bg-black/50 rounded-full">
                      <Crown className="size-4 fill-yellow-500 text-yellow-500"/>
                    </div>
                  )}
                  <div
                    className="opacity-0 group-hover:opacity-100 absolute left-0
